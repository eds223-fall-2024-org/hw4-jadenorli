---
title: '<span style="font-size: 40px; font-weight: bold; text-align: center; display: block;">Homework Assignment Four</span>'
subtitle: '<span style="font-size: 30px; text-align: center; display: block;">Prioritizing Potential Aquaculture</span>'
author: "Jaden Orli"
date: last-modified
bibliography: 
 - references.bib
 - packages.bib
nocite: |
  @tidyverse, @tmap, @here, @janitor, @knitr, @stringr, @tibble, @kableExtra, @RColorBrewer, @htmltools
execute: 
  eval: true
format:
  html: 
...
## c) Potential Finfish Species {#potential_species}

Potential finfish species were selected based on the current commercial fishes managed by the NOAA West Coast operation [@noaa_west_coast]. Highly migratory species (such as sharks) were removed from consideration resulting in a list of 26 [potential finfish species](#load_species). We then used FishBase to compile a table of minimum temperature, maximum temperature, minimum depth, and maximum depth for each species [@fishbase2024]. If a temperature range was not explicity listed, the preferred minimum temperature and preferred maximum temperature (estimated from a model) were recorded [@froese2020]. 


## d) Workflow

The first goal of this assessment, is to determine locations within the US west coast [Economic Exclusion Zones (EEZ)](#oyster_aquaculture) that would be suitable for oyster cultivation. Then a [generalizable workflow](#finfish_aquaculture) was developed to assess the suitability of the 26 potential finfish species previously identified. Finally, the [results](#results) of the analysis are provided.
...
This function can be used to check that the geometric properties (resolution, extent, origin, and crs) of two SpatRaster objects (only compatible with terra) match. This function is a modified version of the [extract_spatial_properties](#extract_spatial_properties) function. 
...
This function can be used to generate a map [@tmap] from a SpatRaster (only comaptible with terra rasters). In this workflow, this function is used to generate a map with the "optimal" and "not optimal" land classifications for each EEZ region for each potential finfish species. 
...
generate_maps <- function(rast_list, map_title){
  ...
  map_title <- map_name_list[i]
  
  #make the map name the species_name + _map
  map_name <- paste0("./maps/", map_title, "_map.png")
...
suitable_locations <- function(raster_obj, min_parameter, max_paramter) {
  ...
  mask_name <- paste0(names(raster_obj), "_mask")
...
The input is:
  ...
  - raster_1: a SpatRaster generated with the [suitable_locations](#suitable_locations) function
  - raster_2: a SpatRaster generated with the [suitable_locations](#suitable_locations) function
...
rasterize_geometries <- function(df, template_raster, name_column) {
  ...
  for(unique_name in name_list) {
    ...
    bbox_name <- paste0(names(unique_name), "_bbox")
...
This function is used to create a mask of a SpatRaster (only compatible with terra) for each layer in a SpatRaster stack. It will then also generate a map for each of the masked layers generated. This function is a modified version of the [rasterize_geometries](#rasterize_geometries) function. In this workflow, this function is used to create a masked SpatRaster for each region in the EEZ with the input raster (raster_obj_input) being the SpatRaster generated for each potential species. The function also CONTAINS the [generate_maps](#generate_maps) function to create the maps for each layer. 
...
Then, define the file paths for the 1. sea surface temperature, 2. bathymetry, and 3. economic exclusion zone data:
  ...
  1. Sea Surface Temperature (SST):
    ...
    - This data was generated by the [NOAA Coral Reef Watch Daily 5km Satellite Sea Surface Temperature (SST) (v3.1)](https://coralreefwatch.noaa.gov/product/5km/index_5km_sst.php).
  2. Bathymetry: 
    - To characterize the depth profile inside the region of interest, we obtained bathymetry data from the [General Bathymetric Chart of the Oceans (GEBCO)](https://www.gebco.net/data_and_products/gridded_bathymetry_data/#area).
  3. Economic Exclusion Zone (EEZ):
    ...
    - This data was developed by the [Flanders Marine Institute](https://www.marineregions.org/eez.php). 
...
2. Bathymetry: 
  ...
  - The bathmetry image file was read in as a SpatRaster with [@terra] containing a depth layer.
...
3. Economic Exclusion Zone (EEZ):
  ...
  - The polygons for the economic exclusion zones were read in as an sf object with [@sf] and the data frame was cleaned with to only contain the regions and the corresponding total area (km^2) while remaining sticky.  
...
We will start by identifying the geometric properties of each of the spatial objects with the [extract_spatial_properties](#spatial_properties) function:
...
This [section](#oyster_aquaculture) identifies locations that would be suitable for only bivalve farming:

  - [a) SST and Bathymetry Data](#sst_bath_data),
  - [b) Determine Suitable Oyster Cultivation Locations](#oyster_range), and 
...
Now we need to verify that the rasters are compatible to be combined. We will start by identifying the geometric properties of with the [extract_spatial_properties](#spatial_properties) function:
...
Now we need to verify that the rasters are compatible to be combined by ensuring the geometric properties have been succesfully transformed with the [check_spatial_properties](#check_spatial) function:
...
To start, we will classify locations as "optimal" or "not optimal" for oyster cultivation based on the [average sea surface temperature](#sst_bath_data) calculated above. This is done by creating a mask of the average sea surface temperature data (mean_sst_celcius) and assigning appropriate categorical levels using the [suitable_locations](#suitable_locations) function: 
...
Now we classified locations as "optimal", "not optimal", or "land" for oyster cultivation based on the [bathymetry data](#sst_bath_data) processed above. This is done by creating a mask of the bathymetry data (depth_resampled) and assigning appropriate categorical levels using the [suitable_locations](#suitable_locations) function: 
...
Once the physical condition rasters have been reclassified, we can combine the reclassified rasters (sst_mask and depth_mask) into a single raster that contains the optimal locations for oyster cultivation based on both parameters using the [optimal_locations](#optimal_locations) function:
...
region_palette <- brewer.pal(n = 5, name = "BuPu")
...
oyster_range_map <- htmltools::tagList(map_title, tmap_leaflet(oyster_range_map))
...
This [section](#finfish_aquaculture) identifies locations that would be suitable for BOTH bivalve and finfish aquaculture:
...
Start by loading in the csv with the metadata for the 26 potential finfish species identified in the [background section](#potential_species):
...
Now we will use the functions defined above to evaluate the compatibility of each potential finfish species in each EEZ region. For each species, we will generate a SpatRaster for each EEZ region whose area has been classified as "optimal" or "not optimal" for the cultivation of that species and oysters. We will then stack these SpatRasters to generate a SpatRaster stack for each species containing a layer for each region. Additionally we will generate a tibble which ranks each EEZ region for each potential finfish species based on the percent of optimal area in the region. Finally, we will also create a map for each region layer generated for each potential species. The functions are used in the below order:
  ...
  - this function is used twice in the workflow; 
    a) the first time is used to find the optimal location for the current species based on the temperature range, and 
    b) the second time is used to find the optimal location for the current species based on the depth range.
  2. [optimal_locations](#optimal_locations),
  
  - this function is used twice in the workflow; 
    a) the first time is used to find the optimal location for the current species based on the depth raster and temperature raster generated, and 
    b) the second time is used to find the optimal location for the current species AND oysters.
  3. [extract_regions](#extract_regions), and
  
  - this function CONTAINS the [generate_maps](#generate_maps) function to create a map for each of the potential finfish species  
  4. [rank_regions](#rank_regions). 
 
Output:
This workflow will generate three outputs:

  1. species_rast_list: a list with 26 sublists (one for each species). Each species sublist contains two more sublists:
    a) rast_list: a list with region specific SpatRasters that are generated for each of the EEZ Regions
    b) rast_stack: a stack of all the SpatRasters that are generated and stored in the rast_list
...
for(i in 1:nrow(potential_species_names)) {
  ...
  #combine the title and the map
  map <- htmltools::tagList(map_title, tmap_leaflet(map))
  
  #add the map to the species_map list
  species_map[[species_name]] <- map
...
This [section](#results) summarizes the results of the above analysis through a variety of visualizations:
...
ggplot(region_rank, aes(x = species, y = region, fill = percent_cover)) +
